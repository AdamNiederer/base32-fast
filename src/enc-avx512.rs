...
pub(crate) fn to_char_avx512<const A: u8>(src: __m512i) -> __m512i {
    let lut = match A {
        Rfc4648 => &RFC4648_LUT,
...
pub(crate) fn b32enc_avx512<'a, const A: u8>(src: &'a [u8], dst: &'a mut [u8]) -> &'a [u8] {
    let mut src_cur = 0;
    let mut dst_cur = 0;
...
mod tests {
    ...
    fn test_rfc4648_avx512() {
        let src = unsafe { _mm512_loadu_si512(TO_CHAR_INPUT.as_ptr() as *const _) };
        let result = to_char_avx512::<Rfc4648>(src);
        let mut actual = [0u8; 64];
        unsafe { _mm512_storeu_si512(actual.as_mut_ptr() as *mut _, result) };
    ...
    fn bench_to_char_avx512(b: &mut Bencher) {
        let input = [0; 64];
        let src_reg = unsafe { _mm512_loadu_si512(input.as_ptr() as *const _) };
        b.iter(|| {
